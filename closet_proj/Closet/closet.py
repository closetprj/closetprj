...
def weather_get():
    ...
    yql_query = "select * from weather.forecast where woeid=12706791"
...
def pushclo(auth_user,watchlistid):
    weather = weather_get()
    teplow = int((int(weather['query']['results']['channel']['item']['forecast'][0]['low']) - 32) / 1.8)
    tephigh = int((int(weather['query']['results']['channel']['item']['forecast'][0]['high']) - 32) / 1.8)
    tmp = 0
    for i in range(0, 7):
        tmp += int(((int(weather['query']['results']['channel']['item']['forecast'][i]['low']) - 32) / 1.8) + int(
            (int(weather['query']['results']['channel']['item']['forecast'][i]['high']) - 32) / 1.8)) / 2
        tmp /= 7
    with sqlite3.connect('user.db') as conn:
        cur = conn.cursor()
        cur.execute(
